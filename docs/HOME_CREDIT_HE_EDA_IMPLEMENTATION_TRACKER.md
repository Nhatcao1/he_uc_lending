...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `app_target_by_income_type` | 5.14.1 Income type by target/default | `application_train.csv` | normalize `NAME_INCOME_TYPE`; create one mask per notebook label; create `TARGET=1` mask | income masks, target mask, context, public key, eval keys | `sum(income_mask)`, `sum(income_mask * target_mask)` | income label, count, percent, default count, default rate | benchmark wrapper uses exact notebook labels |
| `app_target_by_family_status` | 5.14.2 Family status by target/default | `application_train.csv` | normalize `NAME_FAMILY_STATUS`; create one mask per notebook label; create `TARGET=1` mask | family masks, target mask, context, public key, eval keys | `sum(family_mask)`, `sum(family_mask * target_mask)` | family label, count, percent, default count, default rate | benchmark wrapper uses exact notebook labels |
| `app_target_by_occupation_type` | 5.14.3 Occupation by target/default | `application_train.csv` | normalize `OCCUPATION_TYPE`; create one mask per notebook label; create `TARGET=1` mask | occupation masks, target mask, context, public key, eval keys | `sum(occupation_mask)`, `sum(occupation_mask * target_mask)` | occupation label, count, percent, default count, default rate | benchmark wrapper uses exact notebook labels |
| `app_target_by_education_type` | 5.14.4 Education by target/default | `application_train.csv` | normalize `NAME_EDUCATION_TYPE`; create one mask per notebook label; create `TARGET=1` mask | education masks, target mask, context, public key, eval sum key, eval mult key | `sum(education_mask)`, `sum(education_mask * target_mask)` | education label, count, percent, default count, default rate | benchmarked full application table; correctness passed |
| `app_target_by_housing_type` | 5.14.5 Housing type by target/default | `application_train.csv` | normalize `NAME_HOUSING_TYPE`; create one mask per notebook label; create `TARGET=1` mask | housing masks, target mask, context, public key, eval keys | `sum(housing_mask)`, `sum(housing_mask * target_mask)` | housing label, count, percent, default count, default rate | benchmark wrapper uses exact notebook labels |
| `app_target_by_organization_type` | 5.14.6 Organization type by target/default | `application_train.csv` | normalize `ORGANIZATION_TYPE`; create one mask per notebook label; create `TARGET=1` mask | organization masks, target mask, context, public key, eval keys | `sum(organization_mask)`, `sum(organization_mask * target_mask)` | organization label, count, percent, default count, default rate | benchmark wrapper uses exact notebook labels; largest 5.14 case |
| `app_target_by_suite_type` | 5.14.7 Suite type by target/default | `application_train.csv` | normalize `NAME_TYPE_SUITE`; create one mask per notebook label; create `TARGET=1` mask | suite masks, target mask, context, public key, eval keys | `sum(suite_mask)`, `sum(suite_mask * target_mask)` | suite label, count, percent, default count, default rate | benchmark wrapper uses exact notebook labels |
