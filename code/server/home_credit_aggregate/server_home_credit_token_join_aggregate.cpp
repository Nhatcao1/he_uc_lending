...
Options parseArgs(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (arg == "--left-tokens") {
            options.leftTokensPath = needValue(arg);
        }
...
std::set<std::string> readTokenSet(const std::filesystem::path& path) {
    std::ifstream input(path);
    if (!input.is_open()) {
        throw std::runtime_error("cannot open left token file: " + path.string());
    }
    ...
    while (std::getline(input, line)) {
        ...
        const auto token = trim(fields[0]);
    ...
    if (tokens.empty()) {
        throw std::runtime_error("left token set is empty");
    }
    return tokens;
}

std::vector<std::string> readTokenVector(const std::filesystem::path& path) {
    std::ifstream input(path);
    if (!input.is_open()) {
        throw std::runtime_error("cannot open right token file: " + path.string());
    }
    ...
    if (tokens.empty()) {
        throw std::runtime_error("right token vector is empty");
    }
    return tokens;
}
...
std::vector<AggregateRow> readManifest(const std::filesystem::path& path, const std::string& analysisFilter) {
    ...
    if (header != expected) {
        throw std::runtime_error("manifest header must be aggregate manifest header");
    }
    ...
    while (std::getline(input, line)) {
        ...
        if (row.analysis == analysisFilter) {
            rows.push_back(row);
        }
    }
    if (rows.empty()) {
        throw std::runtime_error("manifest contains no rows for requested token-join analysis filter");
    }
    return rows;
}
...
uint32_t maxChunkRows(const std::vector<AggregateRow>& rows) {
    uint32_t value = 0;
    for (const auto& row : rows) {
        value = std::max(value, row.rows);
    }
    if (value == 0) {
        throw std::runtime_error("cannot infer chunk row count");
    }
    return value;
}
...
Ciphertext<DCRTPoly> computeChunk(const Options& options, const CryptoContext<DCRTPoly>& cc, const AggregateRow& row,
    ...
    if (row.operation == "sum") {
        ...
        auto joinedValue = cc->EvalMult(value, joinMask);
        return cc->EvalSum(joinedValue, row.slots);
    }
    if (row.operation == "default_count" || row.operation == "masked_sum") {
        auto value = deserializeCiphertext(options.inputDir / row.valueCiphertext);
        auto product = cc->EvalMultAndRelinearize(joinedMask, value);
...
int main(int argc, char** argv) {
    try {
        ...
        std::cerr << "server_home_credit_token_join_aggregate failed: " << ex.what() << '\n';
        return 1;
    }
}
